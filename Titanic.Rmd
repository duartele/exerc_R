...
Pelos gráficos acima, perebe-se que a **maioria** dos passageiros embarcou **sozinha** ou com apenas um(a) esposo(a)(ou irmã(o)). Com efeito, **apenas 4,35%** dos passageiros possuía 3 ou mais irmãos.
...
Por fim, temos que esta variável apresenta 20.09% de seus valores faltantes. Além disso, há a sugestão de que sua proporção de sobreviventes é **menor** quando comparada à dos outros adultos (**27,76%** contra **38,77%**).

Como algumas técnicas de modelagem não permitem a utilização de "missings", costuma-se contornar esse problema de algumas formas, dentre elas, destacam-se:

1. removendo registros faltantes da base;
2. substituindo os valores faltantes por alguma medida de tendência central (mediana, média ou a moda amostrais);
3. excluíndo a variável do modelo;

No caso em tela, nenhuma dessas alternativas parece ser adequada para nossa situação. Podemos **categorizar** a variável **age** em alguns grupos e um desses seria a representente dos valores faltantes. Outra alternativa é encontrar associações da variável idade com outras variáveis explicativas e só depois disso aplicar medidas de tendência centrais. Então postergaremos a solução deste problema para as próximas seções.
...
Antes de começarmos, precisamos ressaltar que estamos no primeiro semestre, sem termos feito ainda Análise Multivariada. Então os procedimentos aplicados neste capítulo serão guiados por intuições e podem não ser os mais adequados.
...
Pela apreciação desses gráficos, percebe-se os que embarcaram no porto C, que tiveram uma proporção maior de sobreviventes, foram para as três classes, embora haja predominância na primeira. Além disso, vemos que os que embarcaram no porto S também se espalharam pelas três classes. Quanto aos que embarcaram no porto Q, esses foram basicamente para a terceira classe. Portanto, podemos concluir que mesmo que haja associação entre as variáveis, não é algo que forte a ponto dessas variáveis serem redundantes para a modelagem.
...
Há, inclusive, observações que são incoerentes. Por exemplo: por que 8 passageiros da terceira classe pagaram quase 60 valores, sendo que mais de 50 passageiros pagaram menos da metade e foram para a primeira classe? Como não há como verificar se os valores estão incorretos e que são poucas observações, podemos excluir da base essas observações ou simplesmente ignorarmos essas aparentes incoerências.

    3. Variáveis quantitativas

As variáveis quantititativas podem ser analisadas em termos de covariâncias e correlações lineares, inclusive as variáveis **sibsp** e **parch** que, embora discretas, possuem poucos valores diferentes. Iremos apresentar a matriz de correlações entre as variáveis.
...
Vemos, portanto, que as correlações lineares entre as variáveis são baixas. Porém, como nem sempre a associação é linear, vamos analisar os gráficos de dispersões:
...
## Construções de variáveis e sugestções de agrupamentos
...
Também dissemos que, embora não parecesse adequado, poderíamos substituir os valores faltantes da idade por alguma medida de centralidade. Faremos isto também, contudo, chamaremos esse nova variável de **age_2** para não "estragarmos" a variável original.
...
Observando os gráficos e a table summary da variável **fare**, vimos que há apenas 1 valor faltante e que há alguns outiliers que distorcem a escala e, inclusive, podem comprometer a qualidade do modelo. Com efeito, 75% dos valores (do mínimo até o 3° quartil) são inferiores a 32, enquanto há valores que ultrapassam 512!

Vale adiantar, uma das técnicas do KNN é normalizar(fórmula apresentada a seguir) as variáveis para que todas exerceçam a mesma influência no cálculo das distâncias. Esta técnica, contudo, pode ser inviabilizada pela mera existência de um outilier. Para ilustrarmos nossa assertiva, considere os seguintes exemplos:

```{r}
'Suponha que temos 3 variáveis explicativas:
"a"= [1,1,1,2,2,2,3,3,3,4,4,4,5,5,5],
"b"= [10000,10000,10000,20000,20000, 20000,30000,30000,30000,40000,40000,40000,50000,50000, 50000] e "c" = [1,1,1,2,2,2,3,3,3,4,4,4,5,5,500].Sem a normalização, qualquer variação de "a" ou "c" será irrelevante se comparadas com uma variação "b". Ao normalizar, teremos: Normalização de uma variável "x"= (obs(x)-min(x))/(max(x)-min(x))
...
Ou seja, mesmo normalizando as variáveis, a variação de "c" será irrelevante comparadas às outras, pois apenas um outilier conseguiu deixar os outros sendo praticamente 0.
'
```
Então, para contornar esse problema, podemos:
+ 1. categorizar a variável, por exemplo, construindo "fare_agg";
...
Para o procedimento 1, faremos utilizando técnicas de Árvores de Decisão. Para 2, podemos construir uma variável **fare_2**, assim não estragaremos a variável fare, caso não funcione. No tocante ao procedimento 3, poderemos testar isso nas modelagens.
...
Assim, supondo que uma sucessão (Yn) seja independente e identicamente distribuída, teríamos que a soma S = Y1 + Y2 + ... Yn teria uma distribuição Binomial (S ~ Bin(n,p)). Além disso, considerando que a n é grande, podemos usar o TLC (Teorema do Limite Central) e aproximar a distribuição da soma S por uma distribuuição Normal com média np e variância np(1-p).
...
Logo, p_est = 500/1309 aprox.= 0,3820 (Estimativa Pontual). Adotando o grau de confiança de 95%, temos o intervalo de confiança IC = [LI;LS] = [0,3557 ; 0,4083].  
...
Portanto, considerando um nível de significância de 5%, rejetimanos H0, ou seja, há evidências significativas que a proporção de sobreviventes é superior a 33%.
...
Obtemos, portanto o intervalo de confiança (0,4881 ; 0,5848). Como não contem o valor zero, rejeitamos a hipótese nula (proporções de sobrevivência iguais). Além, disso, fica evidente que a proporção de sobreviventes das mulheres **é maior** que a dos homens.

    2.2 **Classe que Embarcou - pclass**
    
Também percebemos que as pessoas que viajaram na primeira classe tiveram uma proporção **maior** de sobreviventes (de **61,92%** contra **25,53%** dos que viajaram na terceira classe). Podemos fazer o teste: H0: p1=p2=p3 vs H1: há alguma diferença entre as proporções. Porém, é mais interessante fazer dois testes: primeiro verificar se p1=p2 e depois verificar se p2=p3.
...
Portanto, rejeitamos H0 ao nível de significância de 5%, ou seja, a proporção de sobreviventes das crianças é dierente (e maior) do que a dos adultos com idade informada. Para testar se proporções de sobreviventes com idade informada e sem idade informada são iguais, verifique o seguinte teste, com o qual concluímos que podemos rejeitar H0:
...
Os outros testes podem ser feitos de forma semelhante com que apresentamos aqui. Então, para não ficar repetitivo, deixemos a cargo do leitor.
...
Primeiro iremos apresentar 3 modelos: **Árvore de Decisão (Simples)**, **Regressão Logística** e o **KNN**. Depois, vamos tentar aumentar a acurácia da previsão, refinando os modelos com algumas técnicas. Para evitarmos o chamado **overfitting**, vamos construir os modelos utilizando uma parte da amostra e com outra parte vamos validar os resultados. Geralmente, é utilizado 70% da amostra para modelagem (ou treinamento) e os outros 30% são destinado à validação.
...
Além disso, ele pode ser usado como ponto de partida para outros modelos, por isto será apresentado primeiro. Vale dizer, ainda, que podemos aplicar a técnica de Random Forest e melhorar ainda mais a performance do modelo, pese o fato de ficar um pouco menos interpretativo.
...
Dos 236 passageiros que não sobreviveram, houve uma acurácia de **89,83%** (212/236) e dos que 157 que sobreviveram tivemos uma acurácia de **60,51%** (95/157). Isto significa que o modelo não consegue classificar bem quem sobreviveu, posto que el erra mais.

Como prometido, vamos agrupar a variável **fare** utilizando uma Árvore de Decisão. Este agrupamento porderá ser utilizado nos próximos modelos. Como estamaos apenas obtendo um agrupamento para uma variável, não tem problema usarmos toda a amostra.    
```{r}
...
Vamos fazer gráficos para ver se este agrupamento faz sentido. Nesse sentido, considere:
...
```{r}
barplot(table(df$survived,df$fare_agg), xlab="Sobrevivência por Tarifa Agg", ylab="Frequência",
        legend.text = TRUE, beside=TRUE)
```

Portanto, este agrupamento tem representatividade nas categorias e ainda manteve o verificado na Análise Descritiva, pois a medida que melhora o grupo, ou seja, se pagam maior valor na tarifa, maior é a proporção de sobreviventes. Baseando-se nesse valor de corte, vamos definir como valor máximo da tarifa sendo 76.
...
Para construírmos um modelo de regressão logística, podemos utilizar a função glm. Para a primeira versão do modelo, vamos começar com as variáveis originais porque as construídas são provenientes delas, exceto pela **age**, pois não podemos ter valores faltantes na Regressão Logística.
...
Contudo, há um pequeno conflito na variável **sibsp**, pois com 1 ou 2 irmãos, a proporção de sobreviventes é maior que a de 0, mas o sinal do coeficiente é negativo. Porém, provalvelmente contornaremos esse problema com as novas variáveis criadas. Por exemplo, a variável **sozinho** pode "punir" quem está com sibsp igual a 0.

Não obstante, antes de tentarmos melhorar a performance do modelo, vamos aplicar este modelo na base de validação para obter a matriz de confusão e, consequentemente, a acurácia do modelo. Assim, saberemos o quanto as novas variáveis podem contribuir na melhora do modelo.
...
Percebe-se que dos 236 (199+37) passageiros que não sobreviveram, acertamos 199 e dos 157 (108+49) que sobreviveram, acertamos 108. Logo, acertamos 307 dos 393 passageiros existentes na base de validação, o que dá uma **acurácia** de **78,12%**. Outra forma de avaliar a qualidade do nosso modelo é verificar se a acurácia dos sobreviventes é próxima da dos não sobreviventes. Olhando a matriz de confusão, a acurácia dos não sobreviventes foi de **84,32%** (199/236) e a dos sobreviventes foi de **68,79%** (108/157), ou seja, nosso modelo não prevê tão bem os que faleceram.
...
Agora que temos as conclusões com as variáveis originais, passemos a tentar melhorar o modelo com as novas variáveis.

As novas variáveis não contribuíram para uma melhora expressiva. A utilização da variável **sozinho** deixou o modelo coerente e ainda fez a acurácia **subir** para **78,62%**. Observemos o resultado:
...
Os resíduos deste modelo continuam bem distribuídos em torno do zero, os coeficientes estão coerentes com o esperado e ainda diminuímos o AIC, então este modelo foi o melhor que conseguimos construir com as variáveis que temos a disposição.
...
Precisamos que todas as variáveis explicativas sejam numéricas. Então, precisaremos fazer alguns outros pré-processamentos (além dos realizados na seção 1). Além disso, assim como na Regressão Logística, vamos trabalhar primeiro com as variáveis originais (novamente não podemos utilizar valores faltantes, então para a idade teremos que usar **age_2** em vez da **age**)
...
Explicamos que as variáveis precisam ser normalizadas. Certamente tem alguma função para fazer isso, mas simplesmente podemos construir uma linha:
...
Este trabalho foi concebido majoritariamente com os conhecimentos adquiridos nos dois primeiros meses do mestrado de Estatística para Ciências de Dados da Universidade do Minho.

Com efeito, para a Análise Descritiva Univariada, muito do conhecimento foi obtido na matéria de Fundamentos e Aplicações de Estatística. No capítulo de Inferência Estatística, todos os testes e códigos do R lá utilizados foram aprendidos na cadeira de mesmo nome. A análise dos "betas" de cada variável foi aprendida na matéria de Modelos Lineares (por analogia aos Beta0 e Beta1 da Regressão Linear). No tocante à modelagem, foi utilizado o conhecimento adquirido na palestra do One Day Meeting patrocinado pela UMinho, que contou ainda com técnicas de Naive Bayes (poderia ter sido construído um modelo deste no presente trabalho). Por fim, o mais importante para a confecção deste trabalho foi obtido na cadeira de Estatística Computacional, pois foi aprendido a utilizar o R e o SPSS (este software também é uma ferramenta fundamental para todos que utilizam a Estatística em seus trabalhos e estudos).
